...
iso="COD"
proj="EPSG:32634" # WGS84 / UTM zone 33N, see http://epsg.io 
extent="-525000 -1550000 1725000 650000" # xmin ymin xmax ymax
tiles_long="39-43" # see http://dwtkns.com/srtm/
tiles_lat="11-15"
...
gdalwarp -overwrite -t_srs $proj -te $extent -r bilinear \
         -tr 90 90 srtm.vrt altitude_.tif
gdal_translate -ot Int16 -co "COMPRESS=LZW" -co "PREDICTOR=2" altitude_.tif altitude.tif
...
gdal_translate -co "COMPRESS=LZW" -co "PREDICTOR=2" _fcc05_10.tif fcc05_10_ctry.tif

# Create raster fcc05_10.tif with 1:for2010, 0:loss05_10
gdal_translate -a_nodata 99 -co "COMPRESS=LZW" -co "PREDICTOR=2" fcc05_10_ctry.tif _fcc05_10.tif # Set nodata different from 255
...
mkdir -p ../data
# Copy files
cp -t ../data fcc05_10.tif dist_*.tif *_UTM.* altitude.tif slope.tif aspect.tif pa.tif AGB.tif
